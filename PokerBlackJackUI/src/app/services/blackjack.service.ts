...
export class BlackjackService {
  ...
  private generateSuitCards(suit: string): PlayingCardModule[] {
    let suitCards: PlayingCardModule[] = [];
  ...
  dealerAutomaticPlay(): void {
    ...
    for (let player of this.players) {
      ...
      if (
        ...
        !player.standing &&
        !player.blackjack &&
        (player.bankroll > 0 || player.currentBetValue > 0)
      ) {
  ...
  private findWinners(): void {
    for (let player of this.players) {
      ...
      if (player.bust) {
        ...
        player.amountWon =
          player.currentBetValue * 2 + player.currentBetValue / 2;
  ...
  private extractCardFromPack(): PlayingCardModule {
    //step 1, extract a random card from the remaining ones in the pack
    let randomCardIndex =
      Math.floor(Math.random() * (this.cardDeck.length + 1)) + 0;
